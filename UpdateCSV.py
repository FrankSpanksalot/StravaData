...
print(gear[["id", "name"]])
vr = df[df["type"] == "VirtualRide"]
print(vr[["id", "name", "type", "gear_id"]])

merged = pd.merge(
    vr,
    gear[["id", "name"]],   # select only relevant columns
    on="gear_id",
    how="left"
)
result = merged[["id", "name", "type", "gear_id", "name_gear"]]
print(result)
